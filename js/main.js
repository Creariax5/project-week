    ...
    cardEl.innerHTML = `
        ...
        <div class="card-image-container">
            <div class="card-image">
                ${card.image ? `<img src="${card.image}" alt="${card.name}" loading="lazy" decoding="async" onerror="this.parentElement.innerHTML='<div class=\"placeholder-image\">${card.icon || '\ud83c\udfcf'}</div>'">` : `<div class="placeholder-image">${card.icon || '🏇'}</div>`}
            </div>
        </div>
        <div class="card-info">
